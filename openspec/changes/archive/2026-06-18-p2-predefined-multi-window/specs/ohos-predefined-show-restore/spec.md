...
### Auto Tests
- `tray.TrayIcon.full_test_tray` — 增加 ShowAll 和 BringAllToFront 菜单项，验证创建/销毁

### Manual Tests
- 点击 Hide → 应用隐藏 → 通过 tray 右键菜单点击 ShowAll → 应用恢复
- 点击 Hide → 应用隐藏 → 通过 tray 右键菜单点击 BringAllToFront → 应用恢复
- 有子窗口时点击 BringAllToFront → 子窗口恢复显示
- 应用前台时点击 ShowAll → 无副作用
