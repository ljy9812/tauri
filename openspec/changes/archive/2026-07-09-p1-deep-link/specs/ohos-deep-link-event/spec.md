...
## Purpose
OHOS 平台 tauri-plugin-deep-link 的编译打通、运行中事件接入、首启动 `get_current` 与命令语义规范：消费 `RunEvent::Opened` 事件链路实现运行中收链接；经 `openharmony-ability` 的 `take_initial_want_uri` getter 实现首启动 `get_current`；定义 `register`/`unregister` no-op、`is_registered` 返回 `Ok(false)`。
