...
## Purpose
OHOS 构建时把 tauri-plugin-deep-link 配置的 scheme/domain 注入 `module.json5` 的 `skills/uris`，让系统能识别并路由 deep link 到 app。定义 `update_ohos_module_json` 注入 API、幂等性、字段映射、home skill 保护规范。
